...
/**
 * Sets up the event listener for the book editing modal (show.bs.modal).
 */
export function initializeEditModal() {
    const editModal = document.getElementById("exampleModal");
    
    // Check if the modal element exists before trying to add a listener
    if (!editModal) {
        console.error("Edit modal element (#exampleModal) not found.");
        return;
    }
