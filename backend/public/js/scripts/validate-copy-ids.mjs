...
// Make sure only CopyID's Uppercase Letters are allowed in the submit fields.
...
const checkCopyIdUniqueness = debounce(async (input, copyId) => {
  try {
    ...
    if (data.exists) {
      ...
      showFeedback(
        input,
        `Copy ID '${copyId}' is already in use by another book.`
      );
    } else {
    ...
    showFeedback(
      input,
      "Could not verify ID. Please check connection or try again."
    );
  }
}, 500);
